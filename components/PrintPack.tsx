  ...
  return (
    <div className="space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm print:hidden">
        ...
        <h1 className="mt-1 text-3xl font-black text-slate-950">涓€鍛ㄦ墦鍗板寘</h1>
        <div className="mt-5"><LevelTabs level={level} onChange={setLevel} /></div>
        <div className="mt-5 flex flex-wrap gap-2">
          {[1, 8, 15, 22, 29, 36, 43, 50].map((day) => <button key={day} onClick={() => setStartDay(day)} className={`rounded-md px-4 py-2 text-sm font-bold ${startDay === day ? "bg-sky-600 text-white" : "bg-slate-100 text-slate-700"}`}>Day {day}-{day + 6}</button>)}
          <button onClick={() => window.print()} className="rounded-md bg-sky-600 px-5 py-2 text-sm font-bold text-white">鎵撳嵃</button>
        </div>
      </section>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-8 shadow-sm print:border-0 print:p-0 print:shadow-none">
        ...
        <div className="mt-6 space-y-8">
          {days.map((day) => {
            const vocab = data.vocabulary.slice(((day - 1) * 5) % data.vocabulary.length, ((day - 1) * 5) % data.vocabulary.length + 5);
            const grammar = data.grammarLessons[(day - 1) % data.grammarLessons.length];
            const questions = data.questions.slice(((day - 1) * 4) % data.questions.length, ((day - 1) * 4) % data.questions.length + 4);
            return (
              <article key={day} className="break-inside-avoid border-t border-slate-300 pt-5">
                ...
                <ol className="mt-1 list-decimal pl-5 text-sm">{vocab.map((word) => <li key={word.id}>{word.word} / {word.chineseMeaning} / {word.japaneseMeaning}</li>)}</ol>
                <p className="mt-3 font-bold">Grammar</p>
                <p className="text-sm">{grammar.title}: {grammar.structure}</p>
                <p className="mt-3 font-bold">Practice</p>
                <ol className="mt-1 list-decimal space-y-2 pl-5 text-sm">{questions.map((q) => <li key={q.id}>{q.question}<br />Answer: ________</li>)}</ol>
        ...
        <div className="mt-8 border-t border-slate-300 pt-5">
          ...
          <p className="mt-2 text-sm leading-7">{data.questions.slice(0, 20).map((q, i) => `${i + 1}. ${q.answer}`).join(" / ")}</p>
